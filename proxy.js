exports.handler = async (event, context) => {
  try {
    ...
    // Decode the HTML from outerQuoteEnd to find the script tag
    const decodedFromEnd = html.substring(outerQuoteEnd).replace(/\\x([0-9A-Fa-f]{2})/g, (_, hex) => String.fromCharCode(parseInt(hex, 16)));
    const scriptStart = decodedFromEnd.indexOf('<script>');
    if (scriptStart === -1) throw new Error("No script tag found after outer userHtml in decoded string");
    const scriptStartRaw = outerQuoteEnd + scriptStart;
    console.log("Script start index (raw):", scriptStartRaw);

    // Find the nested userHtml within the script tag
    const nestedUserHtml = html.indexOf('"userHtml":"', scriptStartRaw);
